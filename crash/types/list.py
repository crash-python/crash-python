...
class TypesListClass(object):
    ...
    def list_for_each(self, list_head):
        ...
        if type(list_head) != gdb.Value:
            raise TypeError("list_head must be gdb.Value representing 'struct list_head' or a 'struct list_head *' not {}".format(type(list_head).__name__))
        if list_head.type == self.list_head_type.pointer():
            list_head = list_head.dereference()
        elif list_head.type != self.list_head_type:
            raise TypeError("Must be struct list_head not {}".format(
                                                        str(list_head.type)))
        fast = None
        ...
        while node.address != list_head.address:
            ...
            try:
                if long(prev.address) != long(node['prev']):
                    pending_exception = CorruptListError("broken prev link {:#x} -next-> {:#x} -prev-> {:#x}".format(
                                                          long(prev.address),
                                                          long(node.address),
                                                          long(node['prev'])))
    ...
    def list_for_each_entry(self, list_head, gdbtype, member):
        for node in list_for_each(list_head):
            if node.type != self.list_head_type.pointer():
                raise TypeError("Type {} found. Expected struct list_head *.".format(str(node.type)))
