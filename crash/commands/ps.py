...
EXAMPLES
  ...
  Search for task names matching a POSIX regular expression:
    ...
    def __init__(self):
        ...
        self.task_states = {
            LinuxTask.TASK_RUNNING         : "RU",
            LinuxTask.TASK_INTERRUPTIBLE   : "IN",
            LinuxTask.TASK_UNINTERRUPTIBLE : "UN",
            LinuxTask.TASK_ZOMBIE          : "ZO",
            LinuxTask.TASK_STOPPED         : "ST",
            LinuxTask.TASK_SWAPPING        : "SW",
            LinuxTask.TASK_EXCLUSIVE       : "EX",
...
# 17077  16749   0  ffff8800b956b848 RU   0.0      0      0  [less]
    ...
    def task_state_string(self, task):
        ...
        if task.TASK_EXCLUSIVE:
            exclusive = (state & task.TASK_EXCLUSIVE) == task.TASK_EXCLUSIVE
            state &= ~task.TASK_EXCLUSIVE
        ...
        if state & task.TASK_DEAD and self.maybe_dead():
            buf = self.task_states[task.TASK_DEAD]
    ...
    def print_one(self, argv, thread):
        ...
        print(line.format(active, task_struct['pid'], parent_pid,
                          task.get_thread_info()['cpu'], int(pointer), width,
